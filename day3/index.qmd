...
But notice that the objects don't "react" to each other:
...
```{r}
# What is the value of y? (remember, y <- x * 10)
```
...
```{r}
# Create a vector of numbers (a "numeric vector")
numbers <- c(1, 2, 3, 4, 5)
numbers
```
...
- Or we could use the comparison to narrow it down to only adults:
...
- This last usage in particular is very helpful for working with data, which will do starting next week!
...
- Edit the `day03-homework.R` file to answer the questions.
